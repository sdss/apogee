...
;-
;------------------------------------------------------------------------------
pro ap_extract_image, fimage, invvar, xcen, sigma, flux, finv, yrow=yrow, $
...
;   xcenuse = transpose(xcen)
   ...
   ; skip the reference pixel rows (1st and last 4 rows)
   if (NOT keyword_set(yrow)) then begin
     yrow = lindgen(ny)
...
;
   ...
   for iy=0, nRowExtract-1 do begin
     ...
     ; Don't fit the reference pixels
     if ny eq 2048 and (cur ge 4 and cur le 2040) then begin
       ...
       pixelmasktemp = 0
       ansrow = ap_extract_row(fimage[*,cur], invvar[*,cur], $
       ...
       ;if cur eq 192 or cur eq 213 then stop
       ...
       ;if cur ge 1803 and cur le 1806 then stop
       ...
       ;; Get widths
       ;;----------
       ;igood = where(mask[*,cur])
       ;widthterm = transpose(ansrow[lindgen(ntrace)*2+1,*])
       ;width = make_array(size=size(fluxrow), /float)
       ;if (igood[0] NE -1) then $
       ; width[igood] = (1 + widthterm[igood] / fluxrow[igood])
       ;
       ;; if sigma was input then you need to multiply "width" by "sigma".
       ;
       ...
       ; why are we printing this really long array to the screen (and clogging the logs) ???
       ; print, cur, niter, djs_median(sigmacur), chisqrow, $
       ; string(13b), format='($, ".",i4.4,i4,f8.2,f8.2,a1)'

       ;stop

     endif ; don't fit reference pixels
