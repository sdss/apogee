...
;------------------------------------------------------------------------------
function ap_reject, ydata, ymodel, outmask=outmask, inmask=inmask, $
 ...
 groupbadpix=groupbadpix, grow=grow, lsigdiff=lsigdiff, usigdiff=usigdiff
  ...
  ; Robust sigma of YDIFF and Median(YDIFF)
  if keyword_set(lsigdiff) or keyword_set(usigdiff) then begin
      if (keyword_set(inmask)) then igood = where(inmask AND outmask, ngood) $
       else igood = where(outmask, ngood)
      if (ngood gt 1) then begin
       sigdiff = mad(ydata[igood] - ymodel[igood])
       meddiff = median(ydata[igood] - ymodel[igood])
      endif else begin
       sigdiff = 0.0
       meddiff = 0.0
      endelse
...
;    message, 'Invalid number of elements for SIGMA'
   ...
   ;----------
   ; Decide how bad a point is according to LSIGDIFF

   if (keyword_set(lsigdiff)) then begin
      qbad =  (ydiff-meddiff) LT -(lsigdiff*sigdiff)
      badness = (-(ydiff-meddiff) / sigdiff * qbad) + badness
   endif

   ;----------
   ; Decide how bad a point is according to USIGDIFF

   if (keyword_set(usigdiff)) then begin
      qbad =  (ydiff-meddiff) GT (lsigdiff*sigdiff)
      badness = ((ydiff-meddiff) / sigdiff * qbad) + badness
   endif
