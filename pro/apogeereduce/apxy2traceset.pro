...
;-
;------------------------------------------------------------------------------
pro apxy2traceset, xpos, ypos, tset, invvar=invvar, func=func, ncoeff=ncoeff, $
 xmin=xmin, xmax=xmax, maxiter=maxiter, inputfunc=inputfunc, $
 inmask=inmask, outmask=outmask, yfit=yfit, inputans=inputans, $
 double=double1, silent=silent, _EXTRA=EXTRA
   ...
   for itrace=0, ntrace-1 do begin
      ...
      while (NOT keyword_set(qdone) AND iiter LE maxiter) do begin
         ...
         qdone = ap_reject(ypos[*,itrace], ycurfit, invvar=tempivar, $
      ...
      ; TWEAK with MPFIT
      res = double(res)
      fa = {func_name:func,xmin:tset.xmin,xmax:tset.xmax}
      fpar = mpfitfun('trace_func',xpos[*,itrace],ypos[*,itrace],tempivar,res,$
                      weights=tempivar*thismask,functargs=fa,yfit=ycurfit2,/quiet)

      ; MPFIT Outlier rejection loop
      endflag = 0
      count=0
      while (endflag eq 0) do begin
        gd = where(thismask eq 1,ngd)
        if ngd eq 0 then begin
          endflag = 1
          goto,BOMB1
        endif
        med = median(ypos[gd,itrace])
        sig = mad(ypos[gd,itrace]) < mad(ypos[gd,itrace]-ycurfit2[gd])  > 0.001
        bd = where( abs(ypos[*,itrace]-ycurfit2) gt med+4*sig and thismask eq 1,nbd)
        if nbd gt 0 then begin
          thismask[gd[bd]] = 0
          fpar = mpfitfun('trace_func',xpos[*,itrace],ypos[*,itrace],tempivar,fpar,$
                        weights=tempivar*thismask,functargs=fa,yfit=ycurfit2,/quiet)
        endif else begin
          endflag = 1
        endelse
        if count ge 10 then endflag=1
        count++
        BOMB1:
      endwhile
      ...
      if keyword_set(pl) then begin
        plot,ypos[*,itrace],/ysty,tit=itrace,/xsty
        bd = where(thismask eq 0,nbd)
        if nbd gt 0 then oplot,bd,ypos[bd,itrace],ps=1,co=150
        oplot,ycurfit2,co=250
        wait,0.3
        ;stop
      endif

      yfit[*,itrace] = ycurfit2
      tset.coeff[*,itrace] = fpar
      ;yfit[*,itrace] = ycurfit 
