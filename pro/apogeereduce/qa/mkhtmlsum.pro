pro mkhtmlsum,apred_vers=apred_vers,all=all,nocheck=nocheck,apstar_vers=apstar_vers,aspcap_vers=aspcap_vers,results_vers=results_vers,mjdmin=mjdmin,mjdmax=mjdmax
...
; MJD web page
openw,html,/get_lun,spectrodir+'/mjd.html'
...
;  file_delete,spectrodir+'index.html.lock',/allow_nonexistent
  fieldfile='fields-lco25m'
  plates=file_search(spectrodir+'/visit/*/*/*/*/'+'*PlateSum*.fits')
endif else begin
  fieldfile='fields'
  plates=file_search(spectrodir+'/apo25m/*/*/'+'*PlateSum*.fits')
endelse
...
printf, html,'<p>Summary files with ASPCAP results:'

printf,html,'<br>aspcap '+aspcap_vers+'/'+results_vers+' allStar <a href=allStar-'+results_vers+'.fits> FITS </a> or '
printf,html,'<a href=allStar-'+results_vers+'.txt> ASCII</a>'
printf,html,' and allVisit <a href=allVisit-'+results_vers+'.fits> FITS </a> or '
printf,html,'<a href=allVisit-'+results_vers+'.txt> ASCII</a>'


printf,html,'<p>In table, light yellow are pre-shutdown'
...
printf,html,'<TABLE BORDER=2 CLASS=sortable>'
printf,html,'<TR><TD>FIELD<TD>Program<TD>ASPCAP<br>'+aspcap_vers+'/'+results_vers+'<TD>PLATE<TD>MJD<TD>LOCATION<TD>RA<TD>DEC<TD>S/N(red)<TD>S/N(green)<TD>S/N(blue)'
...
print,'doing plate: ',plates[i]
 ...
 if iplate[i] ge 0 and status eq 0 then begin
  ...
  if name[iplate[i]] eq oldname then  begin
    ...
    outname=name[iplate[i]]
    outresults='<A HREF='+apstar_vers+'/'+aspcap_vers+'/'+results_vers+'/'+cloc+'>aspcapStar</A><br>'
