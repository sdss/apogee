...
;
; CALLING SEQUENCE:
;       Result = C_correlate(X, Y, Yerr, Lag)
;
; INPUTS:
...
;
;    Yerr:    An n-element vector of the uncertainties/errors in Y.
;
...
;-
function C_Correlate_error, X, Y, Yerr, Lag, Covariance = Covariance, Double = doubleIn
    ...
    if nbdy gt 0 then Yerr2[bdy]=0.0
    ...
    for k = 0L, nLag-1 do begin
        ...
        Cross[k] = (Lag[k] ge 0) ? $
            TOTAL( (Xd[0:nX - Lag[k] - 1L] * Yerr2[Lag[k]:*])^2 ) : $
            TOTAL( (Yerr2[0:nX + Lag[k] - 1L] * Xd[-Lag[k]:*])^2 )
        num[k] = (Lag[k] ge 0) ? $
