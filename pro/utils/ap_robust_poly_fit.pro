FUNCTION AP_ROBUST_POLY_FIT,X,Y,NDEG,YFIT,SIG, NUMIT=THIS_MANY, DOUBLE=DOUBLE, status=status
;+
; NAME:
;	AP_ROBUST_POLY_FIT
;
...
;       =status   The status: 1-okay, 0-bad/problems.
;
...
;  Settle for least-squares:
   LSQFIT = 1
   CC = POLY_FIT( U, V, NDEG, YFIT , DOUBLE=DOUBLE, status=status0)
ENDIF ELSE BEGIN
...
;  Put the leftover points in the middle segment:
   ...
   ENDFOR
;  Now fit:
   CC = POLY_FIT( R,S, NDEG, DOUBLE=DOUBLE, status=status0)
   YFIT = POLY(U,CC)  
ENDELSE
...
; rather than measure errors
  ...
  CC = POLY_FIT( U, V, NDEG, YFIT, MEASURE_ERRORS = 1/W^2, DOUBLE=DOUBLE, status=status1 )
  ISTAT = ROB_CHECKFIT(V,YFIT,EPS,DEL,  SIG,FRACDEV,NGOOD,W,S)
  IF ISTAT EQ 0 THEN GOTO,AFTERFIT
  IF NGOOD LT MINPTS THEN BEGIN
