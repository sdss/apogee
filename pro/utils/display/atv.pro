...
common atv_block, event_block, event_xpos, event_ypos, event_char
...
state = {                   $
        ...
        mousemode: "zoom", $   ; color, blink, zoom, or imexam
        ...
        scaling: 0, $           ; 0=lin,1=log,2=histeq,3=asinh
...
blink_image3 = 0
current_image = 0
saved_image = ptrarr(4,/allocate_heap)
...
if (event.type EQ 5) then begin
  ...
  if event_block eq 1 and $
     eventchar ne 'r' and $
     eventchar ne '-' and $
     eventchar ne '=' then begin
     event_xpos=state.coord[0]
     event_ypos=state.coord[1]
     event_char=eventchar
     if (state.activator EQ 0) then atv_shutdown $
     else state.activator = 0
     return
  endif
...
; Holtz       'r': atv_rowplot, /newcoord
; Holtz       'c': atv_colplot, /newcoord
        ...
        'w': print, state.coord
; Holtz added:
        'y': atv_rowplot, /newcoord
        'x': atv_colplot, /newcoord
        'c': begin
             addstar,state.coord[0],state.coord[1],/centroid
             end
        'j': begin
             addstar,state.coord[0],state.coord[1]
             end
        'r': begin
             p=*saved_image[current_image]
             state.zoom_level =  0
             state.zoom_factor = 1.0
	     atv,p.image,min=p.min,max=p.max,header=p.header,linear=p.linear,log=p.log,histeq=p.histeq,asinh=p.asinh,/noadvance,align=0
             if event_block eq 1 then markstars
             end
        '-': begin
             current_image=(current_image+3) mod 4
             p=*saved_image[current_image]
	     atv,p.image,min=p.min,max=p.max,header=p.header,linear=p.linear,log=p.log,histeq=p.histeq,asinh=p.asinh,/noadvance,/align
             if event_block eq 1 then markstars
             end
        '=': begin
             current_image=(current_image+1) mod 4
             p=*saved_image[current_image]
	     atv,p.image,min=p.min,max=p.max,header=p.header,linear=p.linear,log=p.log,histeq=p.histeq,asinh=p.asinh,/noadvance,/align
             if event_block eq 1 then markstars
             end
        'e': if (state.activator EQ 0) then atv_shutdown $
        else state.activator = 0
        'E': if (state.activator EQ 0) then atv_shutdown $
        else state.activator = 0
; end
; Holtz        'x': atvextract, /newcoord
;  Holtz       'e': atverase
        '!': begin  
            atv_setwindow, state.draw_window_id
            blink_image1 = tvrd(true = true) 
...
; This is atv_activate that will return after a keystroke is hit
...
; Holtz moved following into if statement
;state.coord = round(state.image_size / 2.)
IF (NOT keyword_set(align) OR (state.firstimage EQ 1)) THEN BEGIN
  state.coord = round(state.image_size / 2.)
  state.centerpix = round(state.image_size / 2.)
ENDIF
atv_getoffset
...
; Holtz added
if (not keyword_set(noadvance)) then current_image+=1
if (not keyword_set(log)) then log=0
if (not keyword_set(linear)) then linear=0
if (not keyword_set(histeq)) then histeq=0
if (not keyword_set(asinh)) then asinh=0
if (not keyword_set(minimum)) then minimum=0
if (not keyword_set(maximum)) then maximum=0
if (not keyword_set(header)) then header=0
current_image=current_image mod 4
temp_image = { image: main_image, $
      min: minimum, max: maximum, linear: linear, log: log, histeq: histeq, $
      asinh: asinh, header:header}
*saved_image[current_image] = temp_image
p=*saved_image[current_image]
; end
