{
 ...
 if (newspecid * 1.0 < 100 && substr($0,10,3) == "000" ) printf("'%7.4f             '%5.0f%10i\n",id,ion,nlines)
 else if (newspecid * 1.0 < 100 && substr($0,10,3) != "000" ) printf("'%7.4f             '%5.0f%10i\n",id + substr($0,10,3)/1000,ion,nlines)
 ...
 if (id == 98) type = "CF"
 if (id == 99) type = "ES"
 if (newspecid * 1.0 < 100) printf("'%2s %s'\n",type,iontype)


 while ((getline < inputfile)) {
  if (substr($0,1,1) != "#") {
    specid = substr($0,52,8) * 1.0
    if (specid * 1.0 == substr(newspecid,2,8) * 1.0 ) {
     ...
     if((substr(newspecid,10,3)*1.0 != 0 && iso2*1.0 == substr(newspecid,10,3)*1.0 && isof * 1.0 != 0) || (substr(newspecid,10,3)*1.0 == 0 && isof*1.0 == 0) ) {
      ...
      eastgf = substr($0,47,4)
      EP1 = substr($0,60,12)
      J1 = substr($0,72,5)
      EP1id = substr($0,77,11)
      EP2 = substr($0,88,12)
      J2 = substr($0,100,5)
      EP2id = substr($0,105,11)
      Rad = substr($0,116,6)
      sta = substr($0,122,6)
      vdW = substr($0,128,6)
      org = substr($0,135,3)
      unlte = substr($0,139,2)
      lnlte = substr($0,141,2)
      iso1 = substr($0,143,3)
      hyp = substr($0,146,6) 
      hE1 = substr($0,161,5)
      hE2 = substr($0,166,5)
      F1 = substr($0,171,4)
      note1 = substr($0,175,1)
      S = substr($0,176,1)
      F2 = substr($0,177,4)
      note2 = substr($0,181,1)
      str = substr($0,182,1)
      aut = substr($0,183,3)
      g1 = substr($0,186,5)
      g2 = substr($0,191,5)
      ...
      if (EP2 * 1 < ep * 1 ) {ep = EP2; gu = (J1 * 2.0) + 1}
      if (J2 == "     " || J1 == "     ") gu = 99
      ep = ep * 1.24e-4
      if ( astgf != "       ") gf = astgf + hyp 
      else if ( newgf != "       ") gf = newgf + hyp 
      else gf = orggf + hyp 
      if (newspecid *1.0 < 100) printf("%10.3f %6.3f %7.3f %9.2f %6.1f %9.2e 'x' 'x' \n",lam, ep, gf,vdW, gu, 10^(Rad)-1)
