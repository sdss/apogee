program faltbo
  ...
  real(kind=kind(1.d0)), dimension(600000)         :: lambda,Fabs,Fnorm
...
! we suppose that spectra are always 2 or 3 columns: 1st is lambda, the next 1 or 2 is/are Fnorm and/or Fabs
! we convolve both. BPz 27/07-2011
!
...
!  call system(command)
  ...
  ii=1
  do while (ios == 0)
    read(unit=10,iostat=ios,fmt=*) lambda(ii),Fnorm(ii),Fabs(ii)
    ii=ii+1
...
! loop to convolve 2 spectra (if needed)
  do itype=1,2
    do ii=1,isize
     ...
     if (itype==1) F2(ipad+ii)=Fnorm(ii)
     if (itype==2) F2(ipad+ii)=Fabs(ii)
    end do
...
!     print*,'jbeg jend',jbeg, jend
     ...
     if (itype==1) Fconv1(ii)=somme
     if (itype==2) Fconv2(ii)=somme
     deallocate(dl)
     deallocate(prof)
    end do
  enddo
...
!  print*,'convolution done ! Now writing output ...'
  ...
  do ii=1,isize,resample
    write(20,200) lambda(ii),Fconv1(ii),Fconv2(ii)
  end do
